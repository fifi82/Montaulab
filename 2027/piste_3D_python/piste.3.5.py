...
nb_tour = 2 # nombre de tour à faire
...
def spinCameraTask( task ):
    ...
    # ==========================================
    # MISE À JOUR DU PIP (Vue de dessus)
    # ==========================================
    # Place la caméra PIP au-dessus de la voiture
    pip_camera.setPos(px, py, .5)
    pip_camera.setHpr(0, -90, 0)
    # pip_camera.setHpr(angle, -90, 0)
    ...
    if(save_on): # enregistrement trace (bouge les points de la ligne déjà créée)
        ...
        if (px<-2.487187474966049 and px>-3.084374874830246 and py<-1.289820909500122 and py>-2.378416419029236):
            if (time.time() > t3):
                t3 = time.time() + 10
                n_tour -=1;
                tsl[5].setText(str(nb_tour - n_tour) + " tour(s) / " + str(nb_tour)) # affiche la valeur
                tt2 = time.time()- t2
                tt2 = int(tt2*100)/100
                tsl[4].setText("Chrono = " + str(tt2) + "s ") # affiche la valeur               
                ding.play()
            ...
            if (not n_tour):
                ...
                t2 = time.time()- t2    
                t2 = int(t2*100)/100
                tsl[4].setText("Chrono = " + str(t2) + "s ") # affiche la valeur
                arrivee.play()
    return Task.cont
...
def save_b1(): # bouton sauvegarde
    ...
    if(save_on):
        ...
        t2 = time.time()
        n_tour = nb_tour + 1
        tsl[5].setText(str(nb_tour) + " tour(s)") # affiche la valeur
    else:
...
#voiture = base.loader.loadModel("maillage_3D/fleche.gltf" ) # charge la voiture pour test de colision
voiture.reparentTo(base.render)
voiture.setScale(2.0, 2.0, 2.0)

boom = base.loader.loadSfx("boom.ogg")
arrivee = base.loader.loadSfx("dring.wav")
ding = base.loader.loadSfx("ding.mp3")
...
# Slider 0: rotation caméra + texte tsl[0]
x,y = -1, .9
...
# ==========================================
# CONFIGURATION DE LA CAMÉRA PIP (Vue de dessus)
# ==========================================
# Crée une région d'affichage (gauche, droite, bas, haut) -> Ici en haut à droite
pip_dr = base.win.makeDisplayRegion(0.75, 1.0, 0.75, 1.0)
pip_dr.setSort(10) # S'assure que le PIP est dessiné par-dessus la vue principale

# Crée le noeud de la caméra PIP
pip_cam_node = Camera('pip_camera')

# Utilise un masque de bit pour cacher certains éléments (comme le skybox) de cette caméra
pip_cam_node.setCameraMask(BitMask32.bit(1))

# Utilise une lentille orthographique (pas de perspective, vrai effet 2D de dessus)
pip_lens = OrthographicLens()
pip_lens.setFilmSize(10, 10) # Définit la zone visible (10 mètres de côté autour de la voiture)
pip_lens.setNear(0.01)
pip_lens.setFar(100)
pip_cam_node.setLens(pip_lens)

# Attache la caméra au graph de scène
pip_camera = render.attachNewNode(pip_cam_node)
pip_dr.setCamera(pip_camera)
# Masquer le skybox pour la caméra PIP (pour avoir un fond noir et voir la piste clairement)
#skybox.hide(BitMask32.bit(1))
