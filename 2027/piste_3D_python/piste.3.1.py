...
for x in range(0,mx): # transforme l'image masque en tableau booleen
    for y in range(0,my):
        collision[my-y-1, x] = img[y,x] > 127   # my-y-1 inverse l'image au chargement  
mpx,mpy,zz = 505,285,94 # pour transformation datas réelles en data tableau bool 
...
def spinCameraTask( task ):
    ...
    if(save_on and vd): # enregistrement trace (bouge les points de la ligne déjà créée)
...
def grille_b5(): # bouton bruit tache au sol
    ...
    if(grille_on):
        ...
        b[5].setText(" Grille =  On ") # affiche le texte
    else:
        pls2.hide() # cache la grille
        b[5].setText(" Grille = Off ") # affiche le texte
...
def souris_click():
    if (1): # affiche les coordonnées de la souris
        if souris.hasMouse(): # le pointeur est sur l'écran de Panda3D
            ...
            x,y = souris.getMouseX()*(mpx/zz), souris.getMouseY()*(mpy/zz) # coordonées de l'écran modifier pour s'adapter au sol 3D
            print ( x, y, " /", int(x*zz)+mpx , int(y*zz)+mpy ) #affiche la position de la souris corigé en foction de la vue du haut
...
if(1):
    ...
    for x in range(0,mx,10):
        for y in range(0,my,10): #mx,my
            x1,y1 = (x - mpx)/zz, (y - mpy)/zz # mpx,mpy,zz = 505,285,94
            #x1,y1 = (x - 505)/94, (y - 285)/94 # mpx,mpy,zz = 505,285,94
            ls2.moveTo(x1, y1, 0.01)
            if(collision[y,x]): ls2.setColor(1.0, 0.0, 1.0, 0.0)
...
x,y = 0, .85
b.append( DirectButton(pos = (x+.32,0, y-.01), text=("Grille =  Off"), scale=.035, command=grille_b5) )
