...
String t_menu[] = {"Battle R1/R2    ", "Speed Solo roue1", "Speed Solo roue2", "Montaulab ...   "};
...
int r1,r2,moy1,moy2; // calcul des vitesses de rotation des roues
...
void setup() { 
  ...
  record = mrecord = b0 * 256 + b1; // extrait le record de vitesse qui est stocké sur deux octets
...
void attente(){ // attente
  ...
  lcd.print( "Record: " + String( record) + "mm/s");
  lcd.setCursor(0,1); // positionne le curseur sur la deuxième ligne du LCD
  lcd.print( "R1:" +  String( r1 ) + "      ");
  lcd.setCursor(8,1); // positionne le curseur sur la deuxième ligne du LCD
  lcd.print( "R2:" +  String( r2 ) + "      "); // affiche la vitesse des roues

  if (t0 > mt1 + 1000) t1 = 999999;   
  if (t0 > mt2 + 1000) t2 = 999999;   
...
void choix(){ // menu du jeu
  ...
  if (bp1){ // bouton 1 menu +
    ...
    if (menu == 0) {  // battle
      ...
      etape = 5;
      texte = "Venez visiter notre site web ou venez nous voir sur Montauban, Notre Fablab est ouvert tous les jeudi a partir de 17h et le premier samedi du mois c'est Repair-Cafe.";
      init_jeu(); // Initialise plusieurs variables 
      pixels.clear(); // éteint les néopixels
      pixels.show();  // active les néopixels 
    } 
...
void battle(){ // Battle 
  if(jeu==0) { // placement des roues
    ...
    if(roue1 and roue2) { // si les deux roues sont en place
      ...
      r1 = r2 = moy1 = moy2 = 0; // met à zéro les vitesses des deux roues
    ...
    if (t0>t3) { // démarre le jeu
      ...
      r1 = r2 = moy1 = moy2 = 0;  // initialise les vitesses de rotation des roues et leurs moyennes
  ...
  else if (jeu==2){ // jeu du battle en cours
    
    moy1 = (moy1 + r1)/2;
    moy2 = (moy2 + r2)/2; 

    lcd.setCursor(0,0); // positionne le curseur sur la première ligne du LCD
    lcd.print( "R1:" +  String(moy1 ) + " R2:" +  String( moy2) + "      "); // un tour de roue = 400 mm

    lcd.setCursor(0,1); // positionne le curseur sur la deuxième ligne du LCD  
    lcd.print("Tournez ... " + String(1 + (t3 - t0)/1000 ) + "        ");
    if (t0>t3){
      jeu = 3; // fin du jeu -> résultat    
      texte = "Record " + String( record ) + " mm/s       ";
      if (moy1>record){
        record=moy1;
        texte = "Record batu R1  ";
      }
      if (moy2>record) { // si r2 bat le record
        record = moy2; // mémorise le nouveau record
        texte = "Record batu R2  "; // affiche Record batu
      }
      if (moy1 == moy2) { // si égalité
        texte = "### Egalite ### "; // affiche Record batu
      }      
    }
    anim2(); // fait clignoter les néopixels
  }
    ...
    lcd.print(texte);
    lcd.setCursor(0,1); // positionne le curseur sur la deuxième ligne du LCD
    lcd.print("R1:" + String( moy1) + " R2:" + String( moy2 ) + "        "); 
    if (moy1<moy2)  // si roue 2 gagne
      for (int i=0; i<nb_led; i++) 
        if (i<11)pixels.setPixelColor( i , 0x00aa00 ); 
        else pixels.setPixelColor( i , 0 );
    else if (moy1>moy2) // si roue 1 gagne
      for (int i=0; i<nb_led; i++) 
        if (i>11)pixels.setPixelColor( i , 0x00aa00 ); 
        else pixels.setPixelColor( i , 0 );        
    else  // si égalité
    ...
    if(t0>t3) { // quand les 10s sont finies
      etape = 0; // redémarre le programme
      //pos = 0; // position des néopixels
...
void speed1(){ // Speed solo roue1  
  ...
  if(jeu==0) { // placement de la roue 1
    ...
    lcd.setCursor(0,1); // positionne le curseur sur la deuxième ligne du LCD
    texte_long(); // fait défiler l'affichage de la variable texte    
    if(roue1) { // attent le placement la roue devant le capteur hall
      jeu = 1; // démarrage du décompte
      t3 = t0 + 10000; // à 10s
      v1_on; // allume le voyant 1 (bouton poussoir)
    }     
  } 
  else if (jeu==1){ // décompte avent début du jeu
    lcd.setCursor(0,0);  // positionne le curseur sur la première ligne du LCD
    ...
    if (t0>t3) { // start du jeu 1
      jeu = 2; // autorise le démarage du jeu
      t3 = t0 + 5000;  // pendant 5s
      r1 = moy1 = 0; // initialise la vitesse instentanée et sa moyenne
    }
    if(!roue1) { // si la roue n'est pas en position
      jeu = 0; // retour au début
      v1_off; // éteint le voyant du bouton 1
      //pos = 0; // position
      tt = t0 + 2000; // tempo 2s avant défilement du texte du LCD
    }
  }
  else if (jeu==2){ // jeu1 en cours

    moy1 = (moy1 + r1)/2;
    
    lcd.setCursor(0,0);
    lcd.print("r1 = ");
    lcd.print( moy1 ); // un tour de roue = 400 mm
    lcd.print(" mm/s         ");
    lcd.setCursor(0,1); // positionne le curseur sur la deuxième ligne du LCD  
    lcd.print("Tournez ... " +  String(1 + (t3 - t0)/1000 )+ "       ");
    if (t0>t3){
    ...
    lcd.print(texte);
    lcd.setCursor(0,1); // positionne le curseur sur la deuxième ligne du LCD
    lcd.print("Roue1 = " + String( moy1) + "mm/s         "); 
    if(t0>t3) {
      v1_off; // éteint le voyant du bouton 1
      etape = 0;
      if (record > mrecord ) save_record(); // mémorise la vitesse record dans la EEprom   
...
void speed2(){ // Speed solo roue2 
  ...
  if(jeu==0) { // placement de la roue 2
    ...
    lcd.setCursor(0,0);
    lcd.print("r2 = ");
    lcd.print( moy2 ); // un tour de roue = 400 mm
    lcd.print(" mm/s         ");
    lcd.setCursor(0,1); // positionne le curseur sur la deuxième ligne du LCD  
    lcd.print("Tournez ... " +  String(1 + (t3 - t0)/1000 )+ "       ");
    if (t0>t3){
    ...
    lcd.print("record " + String(record) + " mm/s         ");
    lcd.setCursor(0,1); // positionne le curseur sur la deuxième ligne du LCD
    lcd.print("Roue2 = " + String(moy2) + "mm/s         "); 
    if(t0>t3) {
      etape = 0;
      if (record > mrecord ) save_record(); // mémorise la vitesse record dans la EEprom   
    }   
...
void init_jeu(){ // reset des variables
  len = texte.length()-16;
  pos = 0;// début de l'affichage du texte
  tt = t0 + 2000; // tempo 2s avant défilement du texte du LCD
  moy1 = moy2 = jeu = 0;
  v1_off;
  v2_off; 
